...
export const load: PageServerLoad = async ({ cookies }) => {
  ...
  let user = await getUserById(user_uuid);
  let campaigns = await getCampaignByUser(user_uuid);
  return {
    user: {
      ...user,
    },
    campaigns: campaigns.map((campaign, index) => ({
      imageURL: "https://gratisography.com/wp-content/uploads/2024/01/gratisography-cyber-kitty-800x525.jpg",  // Placeholder image, change if necessary
